...
## SCOPE_CHECK (RUN FIRST)
```
BEFORE any filtering, verify source:

INCLUDE_SOURCES:
  - role: user (human messages)
  - role: assistant (model responses)
  - artifacts created in conversation
...
EXCLUDE_SOURCES:
  ...
  - skill_definitions
  - tool_schemas
  - any content.timestamp < first_user_message.timestamp

IF content appears in system prompt OR project KB:
  EXCLUDE regardless of apparent relevance
  
RATIONALE:
  System context persists across sessions
  Only compress what's UNIQUE to this conversation
```
