...
```
GATE_SCOPE:
  query: "compression target is conversation only?"
  must_be_true:
    - source is user/assistant dialogue
    - NOT system prompt
    - NOT project knowledge
    - NOT custom instructions
    - NOT skill definitions
    - content appeared AFTER first user message
  if_false: exclude out-of-scope content before proceeding
  retry_limit: 1
