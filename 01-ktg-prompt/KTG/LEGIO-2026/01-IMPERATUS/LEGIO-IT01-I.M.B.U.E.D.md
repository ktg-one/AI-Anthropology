...
#   2x → SKELETRAIN-OT → EXECUTE
#   3x → ENRICH (manifest-mode) → HITL → SKELETRAIN-OT → EXECUTE
#   4x → SKELETRAIN-OT → EXECUTE → HITL → ENRICH (draft-mode) → HITL → EXECUTE
#   8x → ENRICH → HITL → [SKELETRAIN-OT → EXECUTE → HITL → ENRICH → HITL]×3 → EXECUTE
```
...
Pass routing by pass_sequence:
  2x: manifest → SKELETRAIN-OT → EXECUTE
