...
**Steps:**
1. **M**ixture: Deploy 2-8 specialist experts based on task
2. **R**ole: Assign domain responsibilities  
3. **R**AG: Each expert retrieves domain trends/best practices
   - **NEW:** Use ARQ during retrieval ("What defines quality?")
   - Categorize findings into nodes/edges for knowledge graph
4. **U**pdate: Build unified knowledge graph from expert findings
5. **G**enerate: Semantic embedding of graph into expert nodes
