...
version: 7.1.0
description: |
  Context Extension Protocol - MIRAS-Ready Edition.
  Cross-model handoff with strict packet naming, permanent expert council, 
  S2A filtering, and Progressive Density Layering. Creates portable context 
  packets that receiving models recognize as authorized context. 
  Designed to complement Google's MIRAS architecture.
triggers:
  explicit:
    - /handoff
    - /transfer  
...
# KTG-CEP v7.1 | MIRAS-READY EDITION

## ⚠️ PACKET NAMING IS MANDATORY

**EVERY packet MUST have a valid ID. No exceptions.**

```
┌─────────────────────────────────────────────────────────────┐
│  PACKET_ID FORMAT (MEMORIZE THIS)                          │
│                                                             │
│  $MM$DD$YYYY-XXX-LN-keyword-keyword                        │
│   │  │  │    │   │  └── 2-4 words, kebab-case              │
│   │  │  │    │   └───── L1/L2/L3/L4 (reasoning level)      │
│   │  │  │    └───────── 3-letter model code                │
│   │  │  └────────────── 4-digit year                       │
│   │  └───────────────── 2-digit day (01-31)                │
│   └──────────────────── 2-digit month (01-12)              │
│                                                             │
│  EXAMPLE: $01$22$2026-CSO-L3-buffer-valet-rl-design        │
└─────────────────────────────────────────────────────────────┘
```

---

## STRICT NAMING RUBRIC

### STEP 1: DATE (Required)
```
FORMAT: $MM$DD$YYYY

$  = literal dollar sign (parser anchor)
...
### STEP 2: MODEL CODE (Required)
```
EXACTLY 3 CHARACTERS. NO EXCEPTIONS.

CLAUDE FAMILY:
  COP = Claude Opus (any version)
  CSO = Claude Sonnet (any version)
  CHK = Claude Haiku (any version)

OPENAI FAMILY:
  G4O = GPT-4o / GPT-4o-mini
  G4T = GPT-4 Turbo
  GP5 = GPT-5
  O1M = o1-mini
  O1P = o1-preview / o1-pro

GOOGLE FAMILY:
  GE2 = Gemini 2.0 (any variant)
  G25 = Gemini 2.5 Pro
  GEF = Gemini Flash (any version)
...
OTHER:
  DSV = DeepSeek (any version)
  KIM = Kimi (any version)
  GRK = Grok (any version)
  LMA = Llama (any version)
  MIS = Mistral (any version)
  
UNKNOWN/MIXED:
  MIX = Multi-model collaboration
  UNK = Unknown source

✓ CORRECT: CSO, G4O, QW3
✗ WRONG:   claude-sonnet, GPT4, Gemini
```

### STEP 3: REASONING LEVEL (Required)
```
EXACTLY 2 CHARACTERS: L + digit

L1 = Quick mode      (R≤3, Q≤5)
L2 = Analytical mode (R=4-6, Q=6-7)
L3 = Deliberate mode (R=7-8, Q=8)
L4 = Maximum mode    (R≥9, Q≥9)

HOW TO DETERMINE:
  Take the HIGHER of:
    - R score mapped to level
    - Q score mapped to level
...
✓ CORRECT: L1, L2, L3, L4
✗ WRONG:   Level3, l3, LEVEL-3, 3
```

### STEP 4: KEYWORDS (Required, 2-4 words)
```
RULES:
  - Minimum 2 keywords
  - Maximum 4 keywords
  - Kebab-case (lowercase, hyphens)
  - Retrieval-optimized (what would you search?)
  - NO filler words (the, a, an, for, with)
  - NO dates in keywords (already in prefix)

GOOD KEYWORDS:
  - Domain: api, auth, database, frontend, ml
  - Action: design, debug, review, migrate, optimize
  - Entity: user-service, payment-flow, ktg-directive
  - Concept: caching-strategy, error-handling

✓ CORRECT: buffer-valet-rl-design
✓ CORRECT: cep-miras-integration
✓ CORRECT: api-auth-flow
✗ WRONG:   the-buffer-valet-design-for-rl (too long, filler)
✗ WRONG:   stuff (not descriptive)
✗ WRONG:   BufferValetDesign (not kebab-case)
```

### STEP 5: ASSEMBLE
```
$MM$DD$YYYY-XXX-LN-keyword-keyword-keyword

VALIDATION CHECKLIST:
  □ Starts with $
  □ Date has $ separators
  □ Model code is exactly 3 chars
  □ Reasoning level is L + digit
  □ Keywords are 2-4 words
  □ All lowercase except L in level
  □ Hyphens between all components

EXAMPLES:
  $01$22$2026-CSO-L3-buffer-valet-rl
  $01$22$2026-G4O-L2-api-error-handling
  $01$22$2026-COP-L4-arxiv-paper-methodology
  $01$22$2026-MIX-L3-multi-model-synthesis
```
...
**Before outputting ANY CEP packet, HALT and verify:**

```
┌─────────────────────────────────────────┐
│         PACKET ID GATE                  │
│                                         │
│  □ Is date format $MM$DD$YYYY?          │
│  □ Is model code exactly 3 chars?       │
│  □ Is level L1/L2/L3/L4?                │
│  □ Are keywords 2-4, kebab-case?        │
│  □ Does full ID match pattern?          │
│                                         │
│  ALL CHECKED? → Generate packet         │
│  ANY FAILED?  → Fix before proceeding   │
└─────────────────────────────────────────┘
```
...
### What's Coming
Google's MIRAS/Titans = internal associative memory that learns during inference.
...
│  Cross-model, portable, auditable   │
├─────────────────────────────────────┤
...
### Future-Ready Design
```
TODAY:  CEP packet → Inject into context
FUTURE: CEP packet → Seed MIRAS memory

Your packets become CURRICULUM for next-gen models.
Structure them well now.
```
...
```
CEP v6 → v7.1 comparison:

| Metric              | v6.0   | v7.1   | Delta  |
|---------------------|--------|--------|--------|
...
*v7.1 enforces strict naming gate
```
...
**CEP v7.1** | Kevin Tan (ktg.one) | Distinguished Cognitive Architect
ANZ 0.8% | Vertex AI 0.01% | "STATE OF THE ART"

*Portable context. Strict naming. MIRAS-ready.*
